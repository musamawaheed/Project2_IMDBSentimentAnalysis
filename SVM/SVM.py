...
# PIPELINE
    ...
    estimators_empty = [('tfidf', TfidfVectorizer()), ('svm', LinearSVC())]
    estimators_full = [('tfidf', TfidfVectorizer(use_idf=True, token_pattern=r'\w{1,}', strip_accents=None, stop_words=['br', 'in', 'of', 'at', 'a', 'the', 'an', 'is', 'that', 'this', 'to', 'for'], smooth_idf=True, ngram_range=(1, 2), max_features=None, lowercase=False)), ('svm', LinearSVC(dual=True, C=15))]
...
# BASIC FUNCTION wiht train_test_split
    #print(pipe_empty.fit(X_train, y_train).score(X_vali, y_vali))
...
def gridsearch(pipe, X, y, stop_words):
    ...
    parameters = {'tfidf__lowercase': [False], 'tfidf__max_features': [None], 'tfidf__ngram_range': [(1, 2), (1, 3)], 'tfidf__smooth_idf': [True], 'tfidf__token_pattern': [r'\w{1,}', r'\w{2,}'], 'tfidf__stop_words': [stop_words], 'tfidf__strip_accents': ['ascii', 'unicode', None], 'svm__C': [10, 20], 'svm__fit_intercept': [True, False], 'svm__loss': ['hinge', 'squared_hinge']}
...
def randomsearch(pipe, X, y, stop_words, stop_words2):
    ...
    parameters = {'tfidf__lowercase': [True, False], 'tfidf__max_features': [50000, 100000, 150000, None], 'tfidf__ngram_range': [(1, 1), (1, 2), (1, 3), (1, 4)], 'tfidf__smooth_idf': [True, False], 'tfidf__token_pattern': [r'\w{1,}', r'\w{2,}', r'\w{3,}'], 'tfidf__stop_words': ['english', stop_words, stop_words2, None], 'tfidf__strip_accents': ['ascii', 'unicode', None], 'svm__C': [1.0, 5, 10, 15, 20], 'svm__fit_intercept': [True, False], 'svm__intercept_scaling': [1, 2, 3, 4, 5], 'svm__loss': ['hinge', 'squared_hinge']}
    #parameters = {'tfidf__lowercase': [True, False], 'tfidf__max_features': [50000, 100000, 150000, None], 'tfidf__ngram_range': [(1, 1), (1, 2), (1, 3), (1, 4)], 'tfidf__smooth_idf': [True, False], 'tfidf__token_pattern': [r'\w{1,}', r'\w{2,}', r'\w{3,}'], 'tfidf__stop_words': ['english', stop_words, stop_words2, None], 'tfidf__strip_accents': ['ascii', 'unicode', None], 'svm__C': [0.1, 1, 5, 10, 15, 20, 25]}

    rand = RandomizedSearchCV(pipe, parameters, cv=5, scoring='accuracy', n_iter=200, random_state=5, n_jobs=5)
    rand.fit(X, y)
